-- Resolve weread-core from the monorepo layout (../../core/lua). When the
-- plugin is packaged standalone, packaging copies core modules into the
-- plugin tree and this entry is simply unused.
do
    local source = debug.getinfo(1, "S").source
    if type(source) == "string" and source:sub(1, 1) == "@" then
        local plugin_dir = source:sub(2):match("(.*/)")
        if plugin_dir then
            package.path = package.path .. ";" .. plugin_dir .. "../../core/lua/?.lua"
        end
    end
end
